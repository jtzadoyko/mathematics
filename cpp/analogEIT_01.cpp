/*
 * File: analogEIT_01.cpp
 *
 * This program integrates...
 *
 * It requires the following files to work (located at same folder):
 *
...
struct rhs_EIT
{
    ...
    {
        dydx[0] = y[1];
        dydx[1] = -gm1*y[1] - w1*w1*y[0] + Omg1*Omg1*y[4] + (F/m1)*cos(w*x);
        dydx[2] = y[3];
        dydx[3] = -gm1*y[3] - w1*w1*y[2] + Omg1*Omg1*y[6] - (F/m1)*sin(w*x);
        dydx[4] = y[5];
        dydx[5] = -gm2*y[5] - w2*w2*y[4] + Omg2*Omg2*y[0];
        dydx[6] = y[7];
...
Int main(void)
{
    ...
    const Doub atol = 1.0E-8;  // Absolute tolerance (error)
    ...
    rhs_EIT EITeqns(1.0E-3);   // Declare d as a rhs_EIT object.
